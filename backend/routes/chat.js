...
router.post('/', async (req, res) => {
  const { projectId, documentId, question } = req.body;
  if (!projectId) {
  ...
  try {
    ...
    const rows = await retrieveContext(projectId, documentId, question, 12);

    if (rows.length === 0) {
      return res.json({
        answer: 'このプロジェクトにはまだ資料がアップロードされていないか、資料のデータを取得できませんでした。資料をアップロードしてから再度お試しください。',
        sources: [],
      });
    }
...
資料抜粋にはプログラムコード(HTML/CSS/JavaScript/Python/Java/C/C++/C#等)が含まれることがあります。コードの処理内容や実装について聞かれた場合は、該当するコードをMarkdownのコードブロック(\`\`\`言語名 ... \`\`\`)でそのまま引用して示し、その上でどのような処理をしているかを説明してください。コードを要約だけして済ませず、実際のコード自体を提示してください。
...
- 回答の最後に根拠箇所(シート名・スライド番号・ファイル名と行番号)を付記してください。
